...
public class Program
{
    public static void Main()
    {
        ...
        for (int x = 0; x < size; x++)
        {
            ...
            {
                if (matrix1[x, y] && matrix2[x, y] && x != start.Item1 && y != start.Item2)
                {
        ...
        {
            var distance = Math.Abs(item.Item1 - start.Item1) + Math.Abs(item.Item2 - start.Item2);
        ...
        Console.WriteLine(intersections.Min(x => CountSteps(start, x, size, input1) + CountSteps(start, x, size, input2)));

        Console.ReadKey();
    }

    public static bool[,] GetMatrix((int x, int y) start, int size, string input)
    {
        ...
        {
            var action = split.First();
            var parameter = int.Parse(new string(split.Skip(1).ToArray()));
            ...
            for (int i = 0; i < parameter; i++)
            {
                ...
                currentPosition.x += rotation.x;
                currentPosition.y += rotation.y;
            }
        }
    ...
    public static int CountSteps((int x, int y) start, (int x, int y) end, int size, string input)
    {
        var result = new bool[size, size];
        var splitted = input.Split(',');
        var currentPosition = (x: start.x, y: start.y);
